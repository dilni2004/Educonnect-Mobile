...
const CreateEditKBItemScreen = ({ route, navigation }) => {
  ...
  const load = async () => {
    try {
      setLoading(true);
      const [categoryData, policyData, itemData] = await Promise.all([
        getAdminKBCategories(),
        getAdminKBPolicies(),
        isEditing ? getKBItem(itemId) : Promise.resolve(null),
      ]);
      const categoryItems = Array.isArray(categoryData) ? categoryData : [];
      const policyItems = Array.isArray(policyData?.items) ? policyData.items : [];
      ...
      if (itemData) {
        setForm({
          ...
          policyId: itemData.policy?.id || itemData.policyId || '',
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <View style={styles.pickerContainer}>
        <Picker selectedValue={form.policyId} onValueChange={(value) => updateField('policyId', value)}>
          <Picker.Item label="No policy" value="" />
          {policies.map((policy) => (
            <Picker.Item key={policy.id || policy._id} label={policy.name} value={policy.id || policy._id} />
          ))}
        </Picker>
      </View>
