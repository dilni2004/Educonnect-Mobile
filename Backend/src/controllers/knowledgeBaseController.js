...
const KnowledgeBasePolicy = require('../models/KnowledgeBasePolicy');
...
const mapItem = (item, includeDetail = false) => {
  ...
  const mappedPolicy = mapPolicy(body.policy);
  const mapped = {
    _id: idOf(body),
    id: idOf(body),
    title: body.title,
    ...
    policy: mappedPolicy,
    policyId: mappedPolicy?.id || idOf(body.policy),
...
const getKnowledgeBaseHome = async (req, res, next) => {
  try {
    const [recommended, trending, featured] = await Promise.all([
      KnowledgeBaseItem.find({ status: 'PUBLISHED', isRecommended: true }).populate('category').populate('policy').sort({ updatedAt: -1 }).limit(6),
      KnowledgeBaseItem.find({ status: 'PUBLISHED' }).populate('category').populate('policy').sort({ updatedAt: -1 }).limit(6),
      KnowledgeBaseItem.find({ status: 'PUBLISHED', isFeatured: true }).populate('category').populate('policy').sort({ updatedAt: -1 }).limit(6),
    ]);
...
const createKnowledgeBaseItem = async (req, res, next) => {
  try {
    const { title, description, type, status, categoryId, content, pdfUrl, policyId, isFeatured, isRecommended } = req.body;
    ...
    if (policyId) {
      ...
      policy = await KnowledgeBasePolicy.findById(policyId);
      if (!policy) {
        return res.status(400).json({ message: 'Policy not found' });
      }
    }
  ...
  try {
    ...
    if (policyId !== undefined) {
      if (!policyId) {
        ...
        if (!isValidObjectId(policyId)) return res.status(400).json({ message: 'Policy not found' });
        const policy = await KnowledgeBasePolicy.findById(policyId);
        if (!policy) return res.status(400).json({ message: 'Policy not found' });
        item.policy = policy._id;
      }
    }
...
const listPolicies = async (req, res, next) => {
  try {
    const policies = await KnowledgeBasePolicy.find().sort({ name: 1 });
...
const listPoliciesForItem = async (req, res, next) => {
  try {
    const item = await findItemById(req.params.id);
    if (!item || !item.policy) return res.json([]);
    return res.json([mapPolicy(item.policy)]);
  } catch (error) {
    return handleControllerError(error, res, next);
  }
};

const listPolicyTemplates = async (req, res) => res.json([
  { id: 'PUBLIC', name: 'Public Access', description: 'Available to authenticated users' },
  { id: 'STAFF_ONLY', name: 'Staff Only', description: 'Available to staff and admins' },
  { id: 'DEPT_ADMIN', name: 'Department Admin', description: 'Available to department administrators' },
]);

const listAdminPolicies = async (req, res, next) => {
  try {
    const policies = await KnowledgeBasePolicy.find().sort({ name: 1 });
    return res.json({ items: policies.map(mapPolicy) });
  } catch (error) {
    return handleControllerError(error, res, next);
  }
};

const getAdminPolicy = async (req, res, next) => {
  try {
    if (!isValidObjectId(req.params.id)) return res.status(404).json({ message: 'Policy not found' });
    const policy = await KnowledgeBasePolicy.findById(req.params.id);
    if (!policy) return res.status(404).json({ message: 'Policy not found' });
    return res.json(mapPolicy(policy));
  } catch (error) {
    return handleControllerError(error, res, next);
  }
};
...
const createPolicy = async (req, res, next) => {
  try {
    ...
    if (!name || !String(name).trim()) return res.status(400).json({ message: 'Policy name is required' });
    const policy = await KnowledgeBasePolicy.create({
      name: String(name).trim(),
      description: description || '',
      rules: rules || '',
      icon: icon || '',
    });
    return res.status(201).json(mapPolicy(policy));
  } catch (error) {
    return handleControllerError(error, res, next);
  }
};
...
const updatePolicy = async (req, res, next) => {
  try {
    ...
    const policy = await KnowledgeBasePolicy.findById(req.params.id);
    if (!policy) return res.status(404).json({ message: 'Policy not found' });
    if (!req.body.name || !String(req.body.name).trim()) return res.status(400).json({ message: 'Policy name is required' });
    policy.name = String(req.body.name).trim();
    policy.description = req.body.description || '';
    policy.rules = req.body.rules || '';
    policy.icon = req.body.icon || '';
    await policy.save();
    return res.json(mapPolicy(policy));
  } catch (error) {
    return handleControllerError(error, res, next);
...
const deletePolicy = async (req, res, next) => {
  try {
    if (!isValidObjectId(req.params.id)) return res.status(404).json({ message: 'Policy not found' });
    const inUse = await KnowledgeBaseItem.exists({ policy: req.params.id });
    if (inUse) return res.status(400).json({ message: 'Policy is in use by knowledgebase items' });
    const deleted = await KnowledgeBasePolicy.findByIdAndDelete(req.params.id);
